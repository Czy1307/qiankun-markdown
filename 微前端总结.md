...
3. sanbox：子应用的隔离，又分为JS的隔离和CSS的隔离
   ...
   [ProxySandBox](https://github.com/umijs/qiankun/blob/master/src/sandbox/proxySandbox.ts)：拦截对`window`上字段的读&写，每个子应用一个沙箱(一个fakewindow)，子应用对window读&写实际是对fakewindow的读写。
